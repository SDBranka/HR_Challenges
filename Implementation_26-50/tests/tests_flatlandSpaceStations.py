...
import sys
sys.path.append("..")
from implementation import flatlandSpaceStations
...
# #47
# https://www.hackerrank.com/challenges/flatland-space-stations/problem?h_r=next-challenge&h_v=zen&h_r=next-challenge&h_v=zen
# Flatland is a country with a number of cities, some of which 
# have space stations. Cities are numbered consecutively and 
# each has a road of  length connecting it to the next city. It 
# is not a circular route, so the first city doesn't connect with
# the last city. Determine the maximum distance from any city to
# its nearest space station.
...
# indices of each city that has a space-station. These values 
...
# 0
# Explanation 1
# In this sample, n = m so every city has space station and we print 0 as
# our answer.